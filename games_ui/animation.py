...
WIN, LOSE, TIE, DRAW = "win", "lose", "tie", "draw"
VALUE_TO_COLOR = {WIN:"darkgreen", LOSE:"darkred", TIE:"goldenrod", DRAW:"yellow"}
...
def CreateArrows():
	...
	for fri in range(ROWS*COLS):
		for toi in range(ROWS*COLS):
			...
			if not(fri == toi or abs(frrow-torow) > 1 or abs(frcol-tocol) > 1): ## neighbors only!
